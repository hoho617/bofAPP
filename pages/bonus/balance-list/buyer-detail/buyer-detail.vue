<template>
	<view class="wrap-container">
				...
				<view class="view-inner">
					...
					<view class="view-content">
						<view class="content-item">
							<text>支付：</text> ￥{{tradingDetail.amount}}
						</view>
						<view class="content-item">
							<text>买入数量：</text> {{tradingDetail.b1}}
						</view>
						<view class="content-item">
		...
		<view class="fixed-btns" v-if="!tradingDetail.imageUrl">
			<view class="write-btns">
			   <view class="write-item" @click="turnToPage">填写汇款信息</view>
			   <view class="write-item" @click="tradingCancel">取消订单</view>
		    </view>
		</view>
		...
		<uni-prompt :show="showPrompt" @cancleEvent="closePrompt" @confirmEvent="cancleConfirm" :tips="'该操作将取消交易'" v-model="message"></uni-prompt>
	</view>
</template>
...
// 				uni.request({
// 					url: this.websiteUrl + 'finance/trading_cancel',
// 					method: 'POST',
// 					data:cancelData,
// 					dataType:'json',//服务器返回json格式数据
// 					header:{'Content-Type':'application/json','Token':this.token},
// 					success: res => {
// 						var res = res.data;
// 						let title = !res.message ? '通证交易取消成功！':res.message;
// 						if(res.code == 200){
// 							console.log("交易取消成功："+JSON.stringify(res.data));
// 							setTimeout(()=>{
// 								uni.showToast({
// 									icon: 'none',
// 									title: title,
