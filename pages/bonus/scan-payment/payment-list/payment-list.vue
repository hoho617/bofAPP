	...
	<view class="scroll-view">
		<scroll-view scroll-y style="height: 100%;" @scrolltolower="loadMore">
			<view class="list-box panel-center bg-white">
				<view class="list-item" v-for="(item,index) in barPayList" :key="index">
					<view class="list-item-left">
						<view class="item-name">{{item.fUserName}}</view>
						<view class="item-memo">{{item.addTime}}</view>
					</view>
				...
				uni.request({
					...
					success: res => {
						...
						if(res.code == 200){
							...
							let list = data.list;
							list.forEach((item)=>{
								item.addTime = this.customMethods.timestampToTime(item.addTime);
							})
							if(flag){
								this.barPayList = this.barPayList.concat(list);
								console.log('取到的data长度',list.length);
							}else {
								this.barPayList = list;
