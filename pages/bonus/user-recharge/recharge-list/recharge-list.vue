			...
			<view class="list-box panel-center bg-white">
				<view class="list-item" v-for="(item,index) in rechargeList" :key="index">
					<view class="list-item-top">
						...
						<view class="item-status">{{item.status == 1 ? '审核通过' : item.status == 2 ? '审核拒绝' : '待审核'}}</view>
					</view>
					<view class="list-item-bottom">
						<view class="item-number">{{item.rechargeSn}}</view>
						<view class="item-money">+{{item.money}}</view>
					</view>
					<view class="list-item-bottom">
						<view class="item-number">备注:{{item.memo?item.memo:''}}</view>
					</view>
				</view>
			</view>
